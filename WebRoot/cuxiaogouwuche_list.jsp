...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<table width="100%" border="0" cellpadding="0" cellspacing="0">
				...
				<td>
					<table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
						<tr align="left" bgcolor="#F2FDFF">
							<td height="30" colspan="17" background="images/table_header.gif">促销购物车列表</td>
						</tr>
					</table>
				</td>
			</tr>
			  ...
			  <table width="100%" border="1" align="center" cellpadding="4" cellspacing="1" bgcolor="#aec3de">
					...
					<tr align="right" bgcolor="#ebf0f7">
					  ...
					  <table width="100%" border="0" align="center" cellpadding="4" cellspacing="1" >
						<tr align="left" >
							<td colspan="17">
								<form action="cuxiaogouwucheList.do" name="myform" method="post">
									...
									<input type="submit" value="查询" class="btn btn-info btn-small" /> <input type="button" value="导出excel" onClick="javascript:location.href='cuxiaogouwuche_listxls.jsp';" class="btn btn-info btn-small" />
								</form>								</td>
						</tr></table>
					  </td>
			    </tr>
				...
				%>
                            <tr align="center">
                             ...
                             <td>${u.wenwanbianhao}</td>    <td>${u.wenwanmingcheng}</td>    <td>${u.wenwanleibie}</td>    <td>${u.yuanjiage}</td>    <td>${u.cuxiaozhekou}</td>    <td>${u.cuxiaojiage}</td>    <td>${u.goumaishuliang}</td>    <td>${u.yingfujine}</td>    <td>${u.maijiazhanghao}</td>    <td>${u.maijiaxingming}</td>    <td>${u.maijiadianhua}</td>    <td>${u.shouhuodizhi}</td>    <td align='center' bgcolor='#ebf0f7'>${u.iszf} </td>    <td align='center'>${u.issh} <input type="button" value="审核" onclick="javascript:location.href='sh.jsp?id=${u.id}&yuan=${u.issh}&tablename=cuxiaogouwuche'"/></td>
                              <td height="50" align="center" > ${u.addtime } </td>
                              <td height="50" align="center"><a class="btn btn-info btn-small" href="doUpdateCuxiaogouwuche.do?id=${u.id }">编辑</a>  <a class="btn btn-info btn-small" href="deleteCuxiaogouwuche.do?id=${u.id }"
										onclick="{if(confirm('确定要删除吗?')){return true;}return false;}">删除</a> <a class="btn btn-info btn-small" href="cxgwcDetail.do?id=${u.id}" target="_blank">详细</a></td>
                            </tr>
                          </c:forEach>
                        </table></td>
						</tr>
		 ...
		 <c:if test="${page.page>1}">
              <a href="cuxiaogouwucheList.do?page=1" >首页</a>
             <a href="cuxiaogouwucheList.do?page=${page.page-1 }"> 上一页</a>             </c:if>
    	     <c:if test="${page.page<page.totalPage}">
			<a href="cuxiaogouwucheList.do?page=${page.page+1 }">下一页</a>
			<a href="cuxiaogouwucheList.do?page=${page.totalPage }">末页</a>		    </c:if>		
	</c:if>
	</p>
